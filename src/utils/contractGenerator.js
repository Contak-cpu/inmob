...
export const generateContract = (contractData, contractType) => {
  ...
  switch (contractType) {
    case 'comercial_ipc':
      template = generateComercialIPCTemplate(contractData);
      break;
    case 'casa_dnu_icl':
      template = generateCasaDNUICLTemplate(contractData);
      break;
    case 'casa_dnu_ipc':
      template = generateCasaDNUIPCTemplate(contractData);
      break;
    case 'casa_empresas':
      template = generateCasaEmpresasTemplate(contractData);
      break;
    default:
...
// Plantilla para Contrato Comercial - IPC
const generateComercialIPCTemplate = (data) => {
  return `
...
// Plantilla para Contrato Casa - DNU ICL
const generateCasaDNUICLTemplate = (data) => {
  return `
CONTRATO DE LOCACIÓN DE VIVIENDA FAMILIAR - DNU ICL
...
TERCERO: Que el precio mensual acordado es de ${formatCurrency(data.monthlyPrice)} 
(${data.monthlyPrice} pesos), con ajuste según DNU ICL.
...
TERCERA - PRECIO: El precio mensual es de ${formatCurrency(data.monthlyPrice)} 
(${data.monthlyPrice} pesos), pagadero por adelantado, con ajuste según DNU ICL.
...
// Plantilla para Contrato Casa - DNU IPC
const generateCasaDNUIPCTemplate = (data) => {
  return `
CONTRATO DE LOCACIÓN DE VIVIENDA FAMILIAR - DNU IPC
...
TERCERO: Que el precio mensual acordado es de ${formatCurrency(data.monthlyPrice)} 
(${data.monthlyPrice} pesos), con ajuste según DNU IPC.
...
TERCERA - PRECIO: El precio mensual es de ${formatCurrency(data.monthlyPrice)} 
(${data.monthlyPrice} pesos), pagadero por adelantado, con ajuste según DNU IPC.
...
// Plantilla para Contrato Casa para Empresas
const generateCasaEmpresasTemplate = (data) => {
  return `
CONTRATO DE LOCACIÓN DE VIVIENDA PARA EMPRESAS
...
SEGUNDO: Que las partes acuerdan celebrar un contrato de locación de vivienda para uso empresarial 
por el plazo de ${data.duration} meses, a partir del ${formatDate(data.startDate)}.

TERCERO: Que el precio mensual acordado es de ${formatCurrency(data.monthlyPrice)} 
(${data.monthlyPrice} pesos), con ajuste específico para empresas.
...
TERCERA - PRECIO: El precio mensual es de ${formatCurrency(data.monthlyPrice)} 
(${data.monthlyPrice} pesos), pagadero por adelantado, con ajuste específico para empresas.
